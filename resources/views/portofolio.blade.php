          ...
          @foreach($portos as $p)
            <article class="relative flex w-full max-w-[26rem] flex-col rounded-xl bg-white bg-clip-border text-gray-700 shadow-lg">
              ...
              <div class="relative mx-4 mt-4 overflow-hidden rounded-xl bg-clip-border text-white shadow-lg">
                ...
                {{-- logo kecil --}}
                <button
                  class="!absolute top-4 right-4 h-8 max-h-[32px] w-8 max-w-[32px] select-none rounded-full text-center align-middle text-purple-500 transition-all hover:bg-purple-500/10 active:bg-purple-500/30"
              ...
              <div class="p-6">
                <div class="mb-3 flex items-center justify-between">
                  <h5 class="block text-xl font-medium leading-snug tracking-normal text-blue-gray-900">
                    {{ $p->name }}
                  </h5>

                  @if($p->formatted_date)
                    <p class="flex items-center gap-1.5 text-base leading-relaxed text-blue-gray-900">
